(ns proserv.operation
  "OperationActor -- one professional-services operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (ProServ-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Professional Services
  ...
  One graph run = one professional-services operation (intake ->
  ...
  decision to a human operator (licensed professional). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:actuation/issue-deliverable` ALWAYS reaches this node when the
  governor is clean -- see `proserv.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [proserv.proservadvisor :as proservadvisor]
            [proserv.governor :as governor]
            [proserv.phase :as phase]
...
(defn build
  ...
  `proserv.store/Store`).
  opts:
    :advisor      -- a `proserv.proservadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (proservadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; ProServ-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (proservadvisor/-advise advisor store request)]
            {:proposal p :audit [(proservadvisor/trace request p)]})))

      ;; Professional Services Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
